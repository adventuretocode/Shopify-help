...
const createSmartCollection = function(body) {
  return new Promise(async function(resolve, reject) {
    ...
    try {
      ...
      console.log(
        "Error: createSmartCollection - ",
        body.smart_collection.title
      );
      reject(error);
    }
  });
...
const createArtistByZTag = function(tag, title) {
  return new Promise(async function(resolve, reject) {
    ...
    try {
      ...
      const {
        smart_collection: { handle, id }
      } = createdSmartCollection;
      await fsWriteFile(
        path.join(
          __dirname,
          `./individual-${NODE_ENV}-${STORE}/${handle}.json`
        ),
        createdSmartCollection
      );
    ...
    for (const tag in tagsAndTitle) {
      try {
        ...
        const message =
          id === "Already Created"
            ? id
            : `\u001b[38;5;${id % 255}mSuccess: ${tag}\u001b[0m`;
        console.log(message);
      } catch (error) {
        console.log("Error: main - ", error, tag);
        const errorJson = require(path.join(
          __dirname,
          createErrorJsonFileName
        ));
        errorJson[tag] = error;
        await fsWriteFile(
          path.join(__dirname, createErrorJsonFileName),
          errorJson
        );
  ...
  return new Promise(async function(resolve, reject) {
    try {
      ...
      await createFileIfNotExist(
        path.join(__dirname, createSuccessJsonFileName)
      );
