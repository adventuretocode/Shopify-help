...
var delete_price_rule = function () {
  fs.readFile(
    `./${process.argv[2]}_price_rule.json`,
    "utf8",
    function (err, body) {
      if (err) throw err;
      ...
      delete_discount(priceRuleId)
        .then(function () {
          postReqDelete(
            `https://zr-dev-btran.myshopify.com/admin/price_rules/${priceRuleId}.json`,
            "Price Rule deleted"
  ...
  return new Promise(function (resolve, reject) {
    fs.readFile(
      ...
      function (err, body) {
        ...
        postReqDelete(
          `https://zr-dev-btran.myshopify.com/admin/price_rules/${priceRuleId}/discount_codes/${discountCodeId}.json`,
          "Discount deleted"
        ).then(function (message) {
          console.log(message);
          resolve();
